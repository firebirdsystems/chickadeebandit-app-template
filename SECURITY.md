...
- [ ] **Value-bearing events.** Does your app *emit* an event another app turns into money, points or state (`reward.earned`)? Any member can POST it to your events endpoint — gate it with `publish_acls`. If members must be able to publish it, the gate cannot protect the NUMBER: publish it from the row instead (`write_effects.<table>.emit`), with the number looked up from a row they cannot write. Does your app *consume* such an event and trust the payload? Restrict to a trusted `source_app_id`. Remember publishing also runs household automations, whose steps execute as trusted SQL that bypasses row policies.
